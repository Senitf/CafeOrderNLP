...
args = easydict.EasyDict({
    "chat": False,
    "sentiment": '0',
    "model_params": 'model_chp/model_last.ckpt',
    "train": False
})
...
class KoGPT2Chat(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 5 == 0:
            writer.add_scalars('loss/training_loss', tensorboard_logs, global_cnt)
            global_cnt += 1
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss_avg = loss.sum() / mask.sum()
        tensorboard_logs = {'test_loss': loss_avg}

        '''
        # calculate acc
        labels_hat = torch.argmax(out, dim=1)
        test_acc = torch.sum(label == labels_hat).item() / (len(label) * 1.0)
        '''
    ...
    def train_dataloader(self):
        data = pd.read_csv('Chatbot_data/data_cafe_train_fixed.csv')
    ...
    def validation_dataloader(self):
        data = pd.read_csv('Chatbot_data/data_cafe_validation_fixed.csv')
        self.val_set = CharDataset(data, self.tok_path, self.vocab, max_len=self.hparams.max_len)
    ...
    def test_dataloader(self):
        data = pd.read_csv('Chatbot_data/data_cafe_test_fixed.csv')
        self.test_set = CharDataset(data, self.tok_path, self.vocab, max_len=self.hparams.max_len)
        test_dataloader = DataLoader(
            self.test_set, batch_size=self.hparams.batch_size, num_workers=2,
            shuffle=True, collate_fn=self._collate_fn)
